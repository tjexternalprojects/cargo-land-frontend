...
function useHome() {
  ...
  const [successfulShipmentLoader, setSuccessfulShipmentLoader] = useState(
    false
  );
  ...
  const successfulShipment = () => {
    ...
    setSuccessfulShipmentLabel(
      state.shipmentSummary.map((obj: any) => obj.month)
    );
    setSuccessfulShipmentData(
      state.shipmentSummary.map((obj: any) =>
        obj.shipmentDetails.some(
          (sd: any) => sd.shipment_Status === "SUCCESSFUL"
        )
