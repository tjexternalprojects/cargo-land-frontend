...
const index = () => {
	...
	return (
		<div className="animate__animated animate__fadeInUp   bg-white md:w-9/12 w-full  md:px-0  p-4 shadow-sm rounded-sm flex flex-wrap md:flex-nowrap items-center justify-center gap-4 mb-3 ">
			<div className=" w-28 h-28 max-h-28 max-w-md rounded-full shadow-md border border-slate-200 relative">
				<img
					src={previewImage ? previewImage : userImg}
					className="w-full h-full object-cover object-top rounded-full"
				/>
				{showLoader && (
					<div className="h-full w-full absolute top-0 left-0 bottom-0 right-0 bg-black/50 rounded-full flex items-center justify-center">
						<RingLoader size={100} loaderColor="#fff" />
					</div>
				)}
			</div>
