...
import { MapDirection, SearchShipmentModal, UpdateShipmentLocation } from "@/components/";
      ...
      {loading ? (
        ...
        <>
          <>
            {showTrackingIdInput ? (
              <SearchShipmentModal
                setShowTrackingIdInput={setShowTrackingIdInput}
              />
            ) : (
              <>
                      ...
                      <div className="text-sm">
                        ...
                        <div className="text-black font-bold border-l-2 pl-6 py-10 border-slate-300 relative">
                          ...
                          singleShipment.shipment_Status === "TRANSIT" ? (
                            <button onClick={()=>setShowUpdateShipmentLocation(true)} className="px-2 py-1 bg-green-700 text-white rounded">
                              Update Location
                            </button>
                          ) : (
                  ...
                  <div className=" w-full py-5">
                    ...
                    <div className="mt-5 ">
                      <MapDirection
                        ...
                        startLocation={{
                          lng: parseFloat(
                            singleShipment?.current_location?.longitude
                          ),
                          lat: parseFloat(
                            singleShipment?.current_location?.latitude
                          ),
                        }}
                        endLocation={{
                          lng: parseFloat(
                            singleShipment?.shipment_destination?.longitude
                          ),
                          lat: parseFloat(
                            singleShipment?.shipment_destination?.latitude
                          ),
                        }}
                      />
                    </div>
               ...
               {showUpdateShipmentLocation && <UpdateShipmentLocation/>}
              </>
            )}
          </>
        </>
